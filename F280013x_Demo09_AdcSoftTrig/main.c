...
//     Enable Global Interrupt (INTM) and realtime interrupt (DBGM)
    ...
    for(;;)
    {
        ...
        AdcaRegs.ADCSOCFRC1.bit.SOC0  = 1 ; //设置软件触发
        while(AdcaRegs.ADCSOCFLG1.bit.SOC0 == 0); //等待采集转换完成
        AdcaRegs.ADCINTFLGCLR.bit.ADCINT1 = 1;    //清除标志位

        RAW_ADCGet();  //读取ADC SOC0原始值

        printf("\r\nADC_A0: %d",RAW_ADC.Raw0);
